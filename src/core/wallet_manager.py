...
class WalletManager:
    ...
    async def initialize(self) -> bool:
        """Initialize wallet manager with proper validation."""
        try:
            # Initialize RPC client
            self.client = AsyncClient(self.rpc_url)
            
            if self.simulation_mode:
                logger.info("Running in simulation mode - no real funds will be used")
                return True
            
            # Validate required credentials for live mode
            if not self._validate_credentials():
                raise WalletSecurityError("Missing or invalid credentials for live mode")
            
            # Initialize real wallet
            await self._initialize_real_wallet()
